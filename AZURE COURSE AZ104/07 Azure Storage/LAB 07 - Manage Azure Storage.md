...
> **Module:** [[07 - Azure Storage]]  ·  **Index:** [[AZ-104 Course Index]]
...
![Diagram of the tasks.](../_attachments/az104-lab07-architecture.png)
...
2. Search for and select `Storage accounts`, select **Storage accounts** in the results, and then click **+ Create**.
...
4. On the **Advanced** and **Security** tabs, use the informational icons to learn more about the choices. Take the defaults.
...
![Screenshot move to cool rule conditions.](../_attachments/az104-lab07-movetocool.png)
...
![Screenshot of create a container.](../_attachments/az104-lab07-create-container.png)

4. On your container, scroll to the ellipsis (...) on the far right, select **Access policy**.

5. If a warning appears stating that authorization with Shared Key is disabled for the account, you can disregard it and continue.

6. In the **Immutable blob storage** area, select **Add policy**, change the type from **Legal hold** to **Time-based retention**.
...
2. Next, navigate to **Access Control (IAM)**, click **Add role assignment**, select the **Storage Blob Data Contributor** role, and assign it to your user account, then click **Review + assign**.

3. Next, do the same steps as in the previous step to assign the **Storage File Data Privileged Contributor** role.

4. Once access is configured,  select your **data** container and then click **Upload**.

5. On the **Upload blob** blade, expand the **Advanced** section.

> [!note]
> Locate a file to upload. This can be any type of file, but a small file is best. A sample file can be downloaded from the AllFiles directory.
...
7. Confirm you have a new folder, and your file was uploaded.

8. Select your upload file and review the ellipsis (...) options including **Download**, **Delete**, **Change tier**, and **Acquire lease**.

9. Select the uploaded file to open its Overview panel, then copy the URL using the **Copy to clipboard** button in the Properties table. Paste the URL into a new **InPrivate** browser window.

10. You should be presented with an XML-formatted message stating **ResourceNotFound** or **PublicAccessNotPermitted**.
...
2. Note the warning banner stating that authorization with Shared Key is disabled for this account — this means the Account key signing option will be unavailable.

3. Specify the following settings (leave others with their default values):Browse back to the file that you uploaded and select the ellipsis (…) to the far right, then select **Generate SAS** and specify the following settings (leave others with their default values):
...
2. Click **+ Classic file share** and on the **Basics** tab give the file share a name, `share1`.
...
6. After the file share is created, an informational banner will appear prompting you to enable backup — you can disregard this message and continue.

![Screenshot of the create file share page.](../_attachments/az104-lab07-create-share.png)
...
2. Select **Clasic file shares** and verify your **share1** directory is present.

3. Select your **share1** directory and notice you can **+ Add directory**. This lets you create a folder structure.

4. If you see an authorization error, select **Switch Azure AD Account** (or change the authentication method to **Microsoft Entra user account**) in the Storage browser toolbar.

5. Select **Upload**. Browse to a file of your choice, and then click **Upload**.
...
2. Under `Virtual networks` click **Create**. On the Basics tab, set **Resource group** as `az104-rg7` and give the virtual network a **name**, `vnet1`.
...
8. Under **Public network access** select **Manage**.

9. Select **Add a virtual network** and then **Add existing network**.

10. Select **vnet1** and **default** subnet, select **Add**.
...
> [!note]
> The storage account should now only be accessed from the virtual network you just created.

13. Select the **Storage browser** and **Refresh** the page. Navigate to your file share or blob content.

> [!note]
> You should receive a message *not authorized to perform this operation*. You are not connecting from the virtual network. It may take a couple of minutes for this to take effect. You may still be able to view the file share, but not the files or blobs in the storage account.


![Screenshot unauthorized access.](../_attachments/az104-lab07-notauthorized.png)
...
Anything conceptual from this lab that belongs in **[[07 - Azure Storage]]** — add it there, not here.
