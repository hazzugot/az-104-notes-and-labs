...
> **Module:** [[10 - Data Protection]]  ·  **Index:** [[AZ-104 Course Index]]
...
![Diagram of the architecture tasks.](../_attachments/az104-lab10-architecture.png)
...
> [!note]
> Take a moment to review the template. We are deploying a virtual network and virtual machine so we can demonstrate backup and recovery.
...
> [!note]
> Wait for the template to deploy, then select **Go to resource**. You should have one virtual machine in one virtual network.
...
![Screenshot of the recovery services vault.](../_attachments/az104-lab10-create-rsv.png)
...
8. Select the **Update** link under **Security Settings > Soft Delete Settings** label.

9. On the **Soft delete Settings** blade, verify that the **Soft delete retention period** is **14** days and close the blade.

> [!tip] Did you know?
> Azure has two types of vaults: Recovery Services vaults and Backup vaults. The main difference is the datasources that can be backed up. Learn more about [the differences](https://learn.microsoft.com/answers/questions/405915/what-is-difference-between-recovery-services-vault).
...
![Screenshot of the backup policy page.](../_attachments/az104-lab10-backup-policy.png)

7. Click **OK** to create the policy and then, in the **Virtual Machines** section, select **Add** (scroll down).
...
9. After the deployment, select **Go to resource**.

10. In the **Protected items** section, click **Backup items**, and then click the **Azure virtual machine** entry.

11. Select the **View details** link for **az104-10-vm0**, and review the values of the **Backup Pre-Check** and **Last Backup Status** entries.

> [!note]
> Notice the backup is pending.

12. Select **Backup now**, accept the default value in the **Retain Backup Till** drop-down list, and click **OK**.
...
3. Use the following information to define the storage account, then select **Review + create**.
...
14. **View details** (scroll to the right for the link) of the backup job.
...
> [!note]
> Make sure that you specify a **different** region than the virtual machine.
...
6. On the **Basics** tab, notice the **Target region**.

7. Select **Next: Advanced settings**. Resource selections have been made for you.

8. Scroll down and **Create** the automation account.

> [!note]
> It is important the settings be populated, or the validation will fail.

9. Select **Review + Start replication** and then **Start replication**.

> [!note]
> Enabling replication will take a 10-15 minutes. Watch the notification messages in the upper right of the portal. While you wait, consider reviewing the self-paced training links at the end of this page.

10. Once the replication is complete, search for and locate your Recovery Services Vault, **az104-rsv-region2**. You may need to **Refresh** the page.

11. In the **Protected items** section, select **Replicated items**.

12. Check that the virtual machine is showing as healthy for the replication health. Note that the status will show the synchronization (starting at 0%) status and ultimately show **Protected** after the initial synchronization completes.

![Screenshot of the replicated items page.](../_attachments/az104-lab10-replicated-items.png)

13. Select the virtual machine to view more details.

> [!tip] Did you know?
> It is a good practice to [test the failover of a protected VM](https://learn.microsoft.com/azure/site-recovery/tutorial-dr-drill-azure#run-a-test-failover-for-a-single-vm).
...
Anything conceptual from this lab that belongs in **[[10 - Data Protection]]** — add it there, not here.
