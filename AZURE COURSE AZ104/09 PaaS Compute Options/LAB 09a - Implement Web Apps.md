...
> **Module:** [[09 - PaaS Compute Options]]  ·  **Index:** [[AZ-104 Course Index]]
...
![Diagram of the tasks.](../_attachments/az104-lab09a-architecture.png)
...
3. Ensure the **Deployment** section of the service menu is expanded, and select **Deployment Center** then **Settings**.

4. If an alert banner appears stating "SCM basic authentication is disabled for your app", select **Enable here** and complete the steps to enable it.

5. In the **Source** drop-down list, select **External Git**. Notice the other choices.

6. In the repository field, enter `https://github.com/Azure-Samples/php-docs-hello-world`

7. In the branch field, enter `master`.

8. Select **Save**.

9. From the staging slot, select **Overview**.

10. Select the **Default domain** link, and open the URL in a new tab.

11. Verify that the staging slot displays **Hello World**.
...
3. In the **Maximum burst** field, select **2**. Set **Minimum instances** to 1.

4. If you have a staging slot, open **Azure Cloud Shell** and run the following command to set the staging slot minimum elastic instance count to 1 before selecting **Save**:
...
![Screenshot of the autoscale page.](../_attachments/az104-lab09a-autoscale.png)

5. Select **Save**.

6. Select **Diagnose and solve problems** (left pane of the web app main page).

7. In the **Load Test your App** box, select **Create Load Test**.

+ Select **+ Create** and give your load test a **name**.  The name must be unique.
+ Select **Review + create** and then **Create**.

8. Wait for the load test to create, and then select **Go to resource**.

9. From the **Overview** | **Create by adding HTTP requests**, select **Create**.

10. On the **Test plan** tab, click **Add request**. In the **URL field**, paste in your **Default domain** URL. Ensure this is properly formatted and begins with **https://**. Select **Add** to save your changes.

11. Select **Review + create** and **Create**.

> [!note]
> It may take a couple of minutes to create the test. Watch the notifications.

12. Navigate to the test (it is listed on the home page).

13. Refresh and review the live metrics including **Virtual users**, **Response time**, and **Requests/sec**.

14. Select **Stop** to initiate the stop request, then select **Stop** again in the confirmation dialog to complete the test run. You don't need to wait for the test to complete.
...
Anything conceptual from this lab that belongs in **[[09 - PaaS Compute Options]]** — add it there, not here.
